<?php
  ...
  function getBaseMapSource($arr_config)
  {
    ...
    for($i=0;$i<$len;$i++)
    {
      if( $arr_config['CONFIG']['MAP']['BASE_MAP'][$i]['@']['enabled']=="true")
      {
        ...
        //return $arr_config['CONFIG']['MAP']['BASE_MAP'][$i]['@']['source'];
      }
    }
    if($temp != "")
      return ltrim($temp,",");
  ...
  function readConfig() {
    ...
    /*VARIABLE IMPORT:: Variables moved from different files*/

    //moved from functions.php after adding config.xml support
    //define("AUTO_DBCOL_PREFIX", "__mlocate__");
    //define("SITE_ADMIN_ROLE", getSiteAdminRole($arr_config));
...
';
    ...
    $js = '<script language="javascript" src="'.$base_path.path_to_theme().'/scripts/serialize.js"></script>
';

    //commented after adding config.xml support
    //$js .= '<script language="javascript">';
    //$js .= 'var config_ser = \''.serialize($ml_config).'\';';
    //$js .= 'var ml_config = unserialize(config_ser);';
    //$js .= '</script>';
    $loadscript .= $js;
